...
* @throws LDAPSearchError
*/
Ldapper.prototype.findGuid = function (guid, attributes, options) {
  var opts = {};
  var appconfig = this._config.getOptions();
  var buffer = null;
  if (Buffer.isBuffer(guid) === true || _.isNull(guid) || _.isUndefined(guid)) {
    ...
    var tmp = helper.unformatGUID(guid);
  ...
  opts['sizeLimit'] = 0;
  opts['paged'] = false;
  var searchDn = (!_.isNull(searchDn) && !_.isUndefined(searchDn)) ? searchDn : appconfig.searchScope;
...
* @throws LDAPSearchError
*/
Ldapper.prototype.findSid = function (sid, attributes, options) {
  var opts = {};
  var appconfig = this._config.getOptions();
  var buffer = null;
  if (Buffer.isBuffer(sid) === true || _.isNull(sid) || _.isUndefined(sid)) {
    ...
    var tmp = helper.unformatSID(sid);
    buffer = new Buffer(tmp, 'hex');
  }
  opts['filter'] = new filters.EqualityFilter({
    attribute: 'objectSid',
...
*/
Ldapper.prototype.delete = function(dn) {
  var appconfig = this._config.getOptions();
  return connectionManager.createClient(appconfig)
  .then(function(client) {
